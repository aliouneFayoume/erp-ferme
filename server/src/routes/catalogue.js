...
module.exports = function catalogueRoutes(pool) {
    ...
    router.post('/produits', requireAuth, checkRole(['admin']), async (req, res) => {
        ...
        try {
            ...
            await logAudit(pool, { table: 'produits', rowId: produit.rows[0].id, action: 'CREATE', userId: req.user.id, tenantId: req.user.tenant_id, details: req.body });
    ...
    router.put('/produits/:id/tarifs', requireAuth, checkRole(['admin']), async (req, res) => {
        ...
        try {
            ...
            await logAudit(pool, { table: 'produits', rowId: req.params.id, action: 'UPDATE', userId: req.user.id, tenantId: req.user.tenant_id, details: req.body });
    ...
    router.put('/produits/:id/stock', requireAuth, checkRole(['admin', 'chef_prod']), async (req, res) => {
        ...
        try {
            ...
            await logAudit(pool, { table: 'stocks', rowId: req.params.id, action: 'UPDATE', userId: req.user.id, tenantId: req.user.tenant_id, details: req.body });
            res.json(result.rows[0]);
        } catch (err) {
            console.error(err);
    ...
    router.delete('/produits/:id', requireAuth, checkRole(['admin']), async (req, res) => {
        await pool.query(`UPDATE produits SET deleted_at = CURRENT_TIMESTAMP WHERE id = $1 AND tenant_id = $2`, [req.params.id, req.user.tenant_id]);
        await logAudit(pool, { table: 'produits', rowId: req.params.id, action: 'DELETE', userId: req.user.id, tenantId: req.user.tenant_id });
        res.status(204).end();
    });
